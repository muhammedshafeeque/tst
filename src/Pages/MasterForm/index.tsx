      ...
      <Box width="40%" sx={{ overflowY: "scroll", ...scrollBarStyle }}>
        <StyledContainer>
          <Box p="10px" borderBottom="1px solid #E5E9EB">
            <Typography
              component={"h2"}
              sx={{ fontSize: 16, fontWeight: 700, color: "#000000" }}
            >
              Select Form
            </Typography>
          </Box>
            ...
            {formValue.map((data, index) => (
              <div key={index}>
                <StyledFormButton onClick={() => handleChange(index)}>
                  <StyledLetterAvatar color={data.iconColor}>
                    R
                  </StyledLetterAvatar>
                  <Stack>
                    <Typography
          ...
          <StyledContainer>
            {!showAddForm ? (
              <Box>
                {!isChecked ? (
                  ...
                  >
                    ...
                    <StyledAddBtn onClick={() => handleAddFormDisplay()}>
                      <AddBox />
              ...
              <Stack p={"15px"} gap="20px">
                ...
                >
                  <Typography
                    sx={{ fontSize: 17, fontWeight: 700, color: "#252525" }}
                  >
                    Add Details
                  </Typography>
                </Stack>
...
function DrapableTable({ checked, handleCheckbox, data }) {
  ...
  const handleSearch = (pageSelect: number) => {
    ...
    if (pageSelect == 1) {
      ...
      skip = perPageValue * (page + pageSelect - 1);
      getData(perPageValue, skip);
    } else {
      if (page !== 1) {
        setPage(page + pageSelect);
        getData(perPageValue, perPageValue * (page + pageSelect - 1));
      } else {
        getData(perPageValue, 0);
      }
    }
  };
  const getData = (limit: number | any, skip: number) => {
    axios
      .get(`machine-test?limit=${limit}&skip=${skip}`)
      .then((res: any) => {
    ...
    <>
      <Table sx={{ border: "1px solid #DDE2E4", borderRadius: "8px" }}>
        <TableHead>
          ...
          >
            <TableCell></TableCell>
            <TableCell>
              <MuiCheckbox checked={checked} onChange={handleCheckbox} />
            </TableCell>
            <TableCell>
              <Stack
                direction="row"
              ...
              >
                <Typography
                  sx={{ fontSize: 13, color: "#5F5F5F", fontWeight: 500 }}
                >
                  Name
                </Typography>
                <Box
            ...
            form.map((item: peopleData, index: number) => (
              ...
              >
                <TableCell sx={{ width: "30px" }}>
                  <DragIndicator
                    sx={{ fontSize: 27, color: "#DDE2E4", cursor: "grab" }}
                  />
                </TableCell>
                ...
                <TableCell
                  ...
                  sx={{
                    width: "80px",
                    button: { color: "#252525", svg: { fontSize: 20 } },
                  }}
                >
      ...
      >
        <Stack direction="row" gap="5px" alignItems="center">
          ...
          <Typography sx={{ color: "#84919A", fontSize: 13, fontWeight: 400 }}>
            Pages
          </Typography>
        </Stack>
        <Stack direction="row" alignItems="center" gap="3px">
          ...
          <Stack direction="row" alignItems="center" gap="8px">
            ...
            <Stack direction="row" alignItems="center" gap="3px">
              <Typography
                sx={{ color: "#344054", fontSize: 13, fontWeight: 500 }}
              >
                Go to page
              </Typography>
              <TextField
                InputProps={{ defaultValue: 2 }}
                sx={{ minWidht: 0, input: { p: "5px", width: "50px" } }}
                type="number"
              />
            </Stack>
          </Stack>
